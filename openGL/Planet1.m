...
-(GLKTextureInfo *) loadTextureFromFile:(NSString *)fileName
{
    ...
    NSDictionary *options = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:YES],GLKTextureLoaderOriginBottomLeft,[NSNumber numberWithBool:YES],GLKTextureLoaderGenerateMipmaps,nil];
    
    NSString *path = [[NSBundle mainBundle] pathForResource:fileName ofType:NULL];
    textureInfo =
    [GLKTextureLoader textureWithContentsOfFile:path options:options error:&error];
...
{    
    ...
    activeTextureIndex = glGetUniformLocation(programObj,"activeTexture");
    
    //load Texture
    textureInfo = [self loadTextureFromFile:fileName];
...
-(bool)execute
{
    ...
    glVertexAttribPointer(positionIndex,3, GL_FLOAT,false, 0, m_VertexData);
    glVertexAttribPointer(colorIndex,4, GL_UNSIGNED_BYTE,true, 0, m_ColorData);
    glVertexAttribPointer(textureCoordinateIndex,2, GL_FLOAT,false, 0, m_TexCoordsData);
